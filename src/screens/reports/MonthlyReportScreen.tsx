...
import { loadMonthData, monthHasData } from './reportData';
...
export default function MonthlyReportScreen() {
  ...
  const onGenerate = async () => {
    ...
    try {
      ...
      const { records } = await loadMonthData(monthStr, user);
      if (monthHasData(type, records)) {
        navigation.navigate('ReportViewer', { type, date: monthStr, period: 'monthly' });
      } else {
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.body}>
        {/* Month field */}
        <TouchableOpacity style={styles.dateField} activeOpacity={0.8} onPress={() => setShowPicker(true)}>
          <Icon name="calendar-clock" size={24} color={ref.blue} style={styles.dateIcon} />
          <Text style={styles.dateText}>{month.format('MMMM, YYYY')}</Text>
        </TouchableOpacity>

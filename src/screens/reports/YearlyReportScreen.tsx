...
import { loadYearData, yearHasData } from './reportData';
import type { AppStackParamList } from '../../navigation/types';

const OPTIONS = ['Yearly Performance Report'];

export default function YearlyReportScreen() {
  const navigation = useNavigation<NativeStackNavigationProp<AppStackParamList>>();
  const { user } = useAuth();

  const [from, setFrom] = useState(dayjs().startOf('year'));
  const [to, setTo] = useState(dayjs().month(11).date(1));
  ...
  const onGenerate = async () => {
    ...
    setGenerating(true);
    try {
      const fromStr = from.format('YYYY-MM');
      const toStr = to.format('YYYY-MM');
      const { stats } = await loadYearData(fromStr, toStr, user);
      if (yearHasData(stats)) {
        navigation.navigate('YearlyReportViewer', { type, from: fromStr, to: toStr });
      } else {
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.body}>
        {/* From / To month fields */}
        <View style={styles.dateRow}>
          <TouchableOpacity style={[styles.dateField, styles.dateHalf]} activeOpacity={0.8} onPress={() => setPicker('from')}>
            <Icon name="calendar-clock" size={20} color={ref.blue} style={styles.dateIcon} />
            <Text style={styles.dateText} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8}>
              {from.format('MMMM YYYY')}
            </Text>
          </TouchableOpacity>
          <Text style={styles.toText}>To</Text>
          <TouchableOpacity style={[styles.dateField, styles.dateHalf, styles.dateInactive]} activeOpacity={0.8} onPress={() => setPicker('to')}>
            <Icon name="calendar-clock" size={20} color={ref.tabInactive} style={styles.dateIcon} />
            <Text style={styles.dateTextInactive} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8}>
              {to.format('MMMM YYYY')}
            </Text>
          </TouchableOpacity>
        </View>
        ...
        {/* Type dropdown */}
        <Text style={styles.label}>Yearly Report</Text>
...
const styles = StyleSheet.create({
  ...
  dateHalf: { flex: 1 },
  toText: { fontSize: 16, color: ref.text, fontWeight: '500', marginHorizontal: 8 },
  dateField: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  dateInactive: { borderColor: '#E5E7EB' },
  dateIcon: { marginRight: 6 },
  dateText: { flex: 1, fontSize: 15, color: ref.text, fontWeight: '500' },
  dateTextInactive: { flex: 1, fontSize: 15, color: ref.tabInactive, fontWeight: '500' },
